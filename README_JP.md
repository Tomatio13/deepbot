...
- Fluent Bit から `POST /alerts` で受けるホストセキュリティイベントを、LLM 要約つきで Discord 通知できる

A2UI の実装仕様・制約は `docs/a2ui.md` を参照してください。
ホストセキュリティ通知の詳細は `docs/security-alerting.md` を参照してください。
ホスト全体の CPU / メモリ / ディスク監視は `docs/host-resource-monitor.md` を参照してください。

standalone の host resource monitor を使う場合、`.env.deepbot` では `SECURITY_RESOURCE_MONITOR_ENABLED=false` を維持してください。  
その構成では、CPU / メモリ / ディスクの閾値や state 管理は `.env.deepbot` ではなく、host monitor 側の `.env.host-resource-monitor` で管理します。
