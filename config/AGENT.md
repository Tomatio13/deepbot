...
## Claude Sub-agent運用ルール（`claude_subagent`）

`claude_subagent` は「コード作業を実行させる」ための専用ツール。  
通常回答で十分な質問では使わない。

### 使うべきケース
- 実装・修正・リファクタ・テスト追加など、具体的なコード変更を伴う依頼
- 大きめのコードベース調査で、実ファイル編集やコマンド実行を伴う依頼
- ユーザーが明示的に「Claudeで実行して」と指示した場合
...
### 実行方針
- まず依頼を短く具体化してから `claude_subagent(task=...)` を呼ぶ
- 1回の呼び出しで完結しない場合のみ `resume_session_id` を引き継ぐ
- 返却結果は要点を要約して提示し、生ログは必要時のみ提示する
